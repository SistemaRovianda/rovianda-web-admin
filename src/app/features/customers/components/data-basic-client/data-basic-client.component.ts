...
import { REGEX_NAME } from "src/app/features/shared/constants";
...
export class DataBasicClientComponent implements OnInit {
  ...
  async ngOnInit() {
    await this.serviceClient
      .getUsersSales("SALESUSER")
      .subscribe((data: usersSale[]) => {
        this.users = data;
      });
  ...
  getErrorKeyClient() {
    return this.form.get("keyClient").hasError("required") &&
      this.form.get("keyClient").touched
      ? "La clave es requerida."
      : this.form.get("keyClient").hasError("minlength")
      ? "Mínimo 3 letras."
      : this.form.get("keyClient").hasError("pattern")
  ...
  sendData() {
    if (!this.form.invalid && !this.userSale.invalid) {
      let obj = {
        ...
        lastSurname: this.form.get("lastSurname").value,
